...
class ExtractHighlightsModal extends Modal {
	...
	async fetchHighlights() {
		...
		res.forEach(x => {
			...
			if (this.settings.includeCreatedDate) {
				text = text + ` - [[${moment(x.bookmark.dateCreated).format(this.settings.dateFormat)}]]`
			}
			...
			if (existingBook) {
				...
				books.set(x.content.bookTitle, new Map<string, string[]>().set(x.content.title, [text]))
			}
		})
